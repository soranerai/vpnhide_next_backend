...
if [[ "$VERSION" == "__unused_android17_fixup__" && -f "$ADDRCONF" ]] &&
   ! grep -q "vpnhide_should_hide_dev(ifa->idev->dev)" "$ADDRCONF"; then
    log "Applying 6.18 inet6_fill_ifaddr fixup in $ADDRCONF..."
    sed -i '/^[[:space:]]*nlh = nlmsg_put(/i\\#ifdef CONFIG_VPNHIDE\n\tif (ifa->idev \&\& ifa->idev->dev \&\&\n\t    unlikely(vpnhide_should_hide_dev(ifa->idev->dev)))\n\t\treturn 0;\n#endif' "$ADDRCONF" \\
fi
